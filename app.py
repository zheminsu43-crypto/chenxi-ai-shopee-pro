...
def explain_gemini_error(
    ...
    if (
        ...
        return (
            "Gemini 模型不存在或你的 API 帳戶無法使用該模型。"
            "系統已避免固定使用 gemini-2.5-flash。"
            f"\n\n原始錯誤：{text}"
        )
    ...
    if (
        ...
        return (
            "Gemini API Key 無效。"
            "請檢查 Streamlit Secrets 裡的 GEMINI_API_KEY。"
            f"\n\n原始錯誤：{text}"
        )
    ...
    if "403" in lower:
        ...
        return (
            "Gemini API 權限不足。"
            "請確認 API Key、Google AI Studio 專案與模型權限。"
            f"\n\n原始錯誤：{text}"
        )
